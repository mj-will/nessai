...
class GWFlowProposal(CPNestFlowProposal):
    ...
    def set_reparameterisations(self, reparameterisations):
        ...
        """
        defaults = dict(mass_inversion=True, flip=False,
                        reduced_quaternions=True,
                        distance_rescaling=False, norm_quaternions=False,
                        rescale_angles=True, euler_convention='ZYZ',
                        angular_decomposition=True, minus_one_to_one=True)
    ...
    def set_rescaling(self):
        ...
        if self.minus_one_to_one:
            self.default_rescaling += \
                list(set(self.names) & set(self.rescaled_names))
        ...
        if self.default_rescaling:
            for n in self.default_rescaling:
                x_prime[n + '_prime'], lj = rescale_minus_one_to_one(
                    x[n], xmin=self.model.bounds[n][0],
        ...
        if self._search_angles:
            for a in self._search_angles.values():
                ...
                if (n := a['radial']) in self.model.names:
                    r, lj = rescale_zero_to_one(
                        x[n], self.model.bounds[n][0], self.model.bounds[n][1])
                    log_J += lj
                else:
                    r = None
                x_prime[a['x']], x_prime[a['y']], lj = angle_to_cartesian(
                    [a['angle']], r=r, scale=a['scale'])
    ...
    def inverse_rescale(self, x_prime):
        ...
        if self._search_angles:
            for a in self._search_angles.values():
                ...
                if (n := a['radial']) in self.model.names:
                    r, lj = inverse_rescale_zero_to_one(
                        r, self.model.bounds[n][0], self.model.bounds[n][1])
                    log_J += lj
                x[a['radial']] = r

        if self.default_rescaling:
            for n in self.default_rescaling:
                x[n], lj = inverse_rescale_minus_one_to_one(
                    x_prime[n + '_prime'],
