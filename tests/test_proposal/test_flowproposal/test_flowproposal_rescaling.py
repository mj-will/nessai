...
def test_set_rescaling_parameters_list(
    proposal, model, update_bounds, rescale_parameters
):
...
def test_rescale_w_reparameterisation(proposal, n):
    ...
    x_prime_out, log_j = FlowProposal._rescale_w_reparameterisation(
...
def test_inverse_rescale_w_reparameterisation(proposal, n):
    ...
    x_out, log_j = FlowProposal._inverse_rescale_w_reparameterisation(
        proposal, x_prime
    )
...
def test_rescale_to_bounds(proposal, model, n, compute_radius):
    ...
    x_prime, log_j = FlowProposal._rescale_to_bounds(
...
def test_rescale_to_bounds_w_inversion_duplicate(
    ...
    with patch(
        "nessai.proposal.flowproposal.detect_edge", return_value=bound
    ) as mock_detect_edge:
        x_prime, log_j = FlowProposal._rescale_to_bounds(
    ...
    np.testing.assert_array_equal(
        mock_detect_edge.call_args[0][0], x_prime_expected["x_prime"][:n]
    )
...
@pytest.mark.parametrize("n", [1, 10])
@pytest.mark.parametrize("bound", ["lower", "upper"])
def test_rescale_to_bounds_w_inversion_split(proposal, model, n, bound):
    ...
    with patch(
        "nessai.proposal.flowproposal.detect_edge", return_value=bound
    ) as mock_detect_edge, patch("numpy.random.choice", return_value=inv):
        x_prime, log_j = FlowProposal._rescale_to_bounds(
...
@pytest.mark.parametrize("n", [1, 10])
@pytest.mark.parametrize("bound", ["lower", "upper"])
def test_rescale_to_bounds_w_inversion_false(proposal, model, n, bound):
    """
...
def test_inverse_rescale_to_bounds(proposal, model, n):
    ...
    model.bounds["z"] = [-10, 10]
    x_prime = numpy_array_to_live_points(
        np.random.randn(n, 3), ["x_prime", "y_prime", "z"]
    ).squeeze()
    x_expected = numpy_array_to_live_points(np.zeros([n, 3]), ["x", "y", "z"])
    ...
    x, log_j = FlowProposal._inverse_rescale_to_bounds(proposal, x_prime)
...
def test_inverse_rescale_to_bounds_w_inversion(proposal, model, n, itype):
    ...
    model.bounds["z"] = [-10, 10]
    x_prime = numpy_array_to_live_points(
        np.random.uniform(-1, 1, (n, 3)), ["x_prime", "y_prime", "z"]
    ).squeeze()
