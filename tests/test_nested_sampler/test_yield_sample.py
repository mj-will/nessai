...
def test_yield_sample_accept(sampler, old_sample):
    ...
    count, next_sample = next(sampler.yield_sample(old_sample))
...
def test_yield_sample_not_populated(sampler):
    """Test function when sample is rejected and a new sample is not drawn"""
    old_sample = parameters_to_live_point([5, 5], names=sampler.model.names)
