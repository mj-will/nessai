...
@pytest.fixture(params=flows)
def conditional_flow(request, data_dim, context_features):
    return request.param(
        data_dim, 8, 2, 2, context_features=context_features
    ).eval()
...
def test_sample_and_log_prob_conditional(
    conditional_flow, n, data_dim, context_features
):
    """Test method for conditional flows."""
    c = torch.randn(n, context_features)
    with torch.no_grad():
